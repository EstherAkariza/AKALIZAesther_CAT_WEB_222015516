    ...
    if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
        ...
        if (in_array($image_type, $allowed_extensions)) {
            ...
            if (move_uploaded_file($image_tmp, $destination)) {
                ...
                $sql = "INSERT INTO users (fullName, email, address, phone, password, age, gender, image, status,role) VALUES ('$fullName', '$email', '$address', '$phone', '$password', '$age', '$gender', '$image', '$status','nurse')";
                if ($conn->query($sql) === TRUE) {
                    // Redirect to manage nurses page with success message
                    header("Location: ./?page=nurses&message=insert");
                    exit();
                } else {
